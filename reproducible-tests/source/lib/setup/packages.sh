...
ensure_curses() {
    ...
    as_root zypper --non-interactive install -y python3-curses 2>/dev/null \
...
check_required_tools_step() {
    ...
    substep_run "python3 curses module" ensure_curses \
        || die_with_next "Failed to load the curses Python module." "install python3-curses, then rerun: bash setup_xdp.sh --force ${IFACES[*]}"
