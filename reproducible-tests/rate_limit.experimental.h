...
/* Build-time knobs used by the verifier benchmark.  The zero values preserve
 * the production layout and accounting semantics. */
#ifndef AUTO_XDP_SHARED_CONN_UPDATE_MODE
#define AUTO_XDP_SHARED_CONN_UPDATE_MODE 0
#endif
#ifndef AUTO_XDP_TCP_ACCOUNTING_MODE
#define AUTO_XDP_TCP_ACCOUNTING_MODE 0
#endif
#ifndef AUTO_XDP_TCP_ACCOUNT_L3
#define AUTO_XDP_TCP_ACCOUNT_L3 1
#endif
#ifndef AUTO_XDP_TCP_ACCOUNT_L4
#define AUTO_XDP_TCP_ACCOUNT_L4 1
#endif
#ifndef AUTO_XDP_TCP_ACCOUNT_L5
#define AUTO_XDP_TCP_ACCOUNT_L5 1
#endif
#ifndef AUTO_XDP_TCP_ACCOUNT_FAMILY_SPLIT
#define AUTO_XDP_TCP_ACCOUNT_FAMILY_SPLIT 0
#endif

#if AUTO_XDP_SHARED_CONN_UPDATE_MODE == 0
#define SHARED_CONN_UPDATE_ATTR static __always_inline
#elif AUTO_XDP_SHARED_CONN_UPDATE_MODE == 1
#define SHARED_CONN_UPDATE_ATTR static inline
#elif AUTO_XDP_SHARED_CONN_UPDATE_MODE == 2
#define SHARED_CONN_UPDATE_ATTR static __attribute__((noinline))
#elif AUTO_XDP_SHARED_CONN_UPDATE_MODE == 3
#define SHARED_CONN_UPDATE_ATTR __attribute__((noinline))
#else
#error "invalid AUTO_XDP_SHARED_CONN_UPDATE_MODE"
#endif

#if AUTO_XDP_TCP_ACCOUNTING_MODE == 0
#define TCP_ACCOUNTING_ATTR static __always_inline
#elif AUTO_XDP_TCP_ACCOUNTING_MODE == 1
#define TCP_ACCOUNTING_ATTR static __attribute__((noinline))
#elif AUTO_XDP_TCP_ACCOUNTING_MODE == 2
#define TCP_ACCOUNTING_ATTR __attribute__((noinline))
#else
#error "invalid AUTO_XDP_TCP_ACCOUNTING_MODE"
#endif
...
SHARED_CONN_UPDATE_ATTR int shared_conn_count_update(
...
#if !AUTO_XDP_TCP_ACCOUNT_FAMILY_SPLIT
TCP_ACCOUNTING_ATTR int tcp_src_conn_record_established(
...
TCP_ACCOUNTING_ATTR int tcp_src_conn_record_activity(struct flow_key *key, __u64 now,
                                                     __u32 dest_port)
{
#if !AUTO_XDP_TCP_ACCOUNT_L3 && !AUTO_XDP_TCP_ACCOUNT_L4 && !AUTO_XDP_TCP_ACCOUNT_L5
    return 0;
#endif
#if AUTO_XDP_TCP_ACCOUNT_L3
    if (key->family == CT_FAMILY_IPV4) {
...
TCP_ACCOUNTING_ATTR int tcp_src_conn_record_close(struct flow_key *key, __u64 now,
                                                  __u32 dest_port)
{
#if !AUTO_XDP_TCP_ACCOUNT_L3 && !AUTO_XDP_TCP_ACCOUNT_L4 && !AUTO_XDP_TCP_ACCOUNT_L5
    return 0;
...
#else
/* Family-specialized entry points keep the IPv4/IPv6 decision out of each
 * accounting layer.  This is useful when measuring verifier state pruning. */
static __always_inline void tcp_account_established_v4(
    struct flow_key *key, __u64 now, __u32 dest_port, struct xdp_runtime_cfg *cfg)
{
    __u64 timeout_ns = cfg_tcp_timeout_ns(cfg);
#if AUTO_XDP_TCP_ACCOUNT_L3
    struct tcp_src_conn_key_v4 skey;
    ...
    sv = bpf_map_lookup_elem(&tsc4, &skey);
    if (!sv) {
        struct tcp_src_conn_val value;
        __builtin_memset(&value, 0, sizeof(value));
        value.state = conn_state_pack(conn_state_tick(now), 1);
        if (bpf_map_update_elem(&tsc4, &skey, &value, BPF_NOEXIST) != 0)
            sv = bpf_map_lookup_elem(&tsc4, &skey);
    }
    if (sv)
        shared_conn_count_record(&sv->state, now, timeout_ns);
#endif
#if AUTO_XDP_TCP_ACCOUNT_L4
    {
        struct tcp_port_policy_cfg *policy =
        ...
        struct prefix_rate_key_v4 pkey;
        struct tcp_pfx_conn_val *pv;
        fill_prefix_rate_key_v4(&pkey, key, dest_port,
                                policy ? policy->source_prefix_v4 : 32);
        pv = bpf_map_lookup_elem(&tsc_pfx4, &pkey);
        if (!pv) {
            struct tcp_pfx_conn_val value;
            __builtin_memset(&value, 0, sizeof(value));
            value.state = conn_state_pack(conn_state_tick(now), 1);
            if (bpf_map_update_elem(&tsc_pfx4, &pkey, &value, BPF_NOEXIST) != 0)
                pv = bpf_map_lookup_elem(&tsc_pfx4, &pkey);
        }
        if (pv)
            shared_conn_count_record(&pv->state, now, timeout_ns);
    }
#endif
#if AUTO_XDP_TCP_ACCOUNT_L5
    {
        struct tcp_port_conn_val *pv = bpf_map_lookup_elem(&tsc_port, &dest_port);
        if (pv)
            shared_conn_count_record(&pv->state, now, timeout_ns);
    }
#endif
...
static __always_inline void tcp_account_established_v6(
    struct flow_key *key, __u64 now, __u32 dest_port, struct xdp_runtime_cfg *cfg)
{
    __u64 timeout_ns = cfg_tcp_timeout_ns(cfg);
#if AUTO_XDP_TCP_ACCOUNT_L3
    ...
    if (!sv) {
        ...
        __builtin_memset(&value, 0, sizeof(value));
        value.state = conn_state_pack(conn_state_tick(now), 1);
        if (bpf_map_update_elem(&tsc6, &skey, &value, BPF_NOEXIST) != 0)
            sv = bpf_map_lookup_elem(&tsc6, &skey);
    }
    if (sv)
        shared_conn_count_record(&sv->state, now, timeout_ns);
...
#if AUTO_XDP_TCP_ACCOUNT_L4
    {
        ...
        struct prefix_rate_key_v6 pkey;
        struct tcp_pfx_conn_val *pv;
        fill_prefix_rate_key_v6(&pkey, key, dest_port,
        ...
        if (!pv) {
            ...
            __builtin_memset(&value, 0, sizeof(value));
            value.state = conn_state_pack(conn_state_tick(now), 1);
            if (bpf_map_update_elem(&tsc_pfx6, &pkey, &value, BPF_NOEXIST) != 0)
                pv = bpf_map_lookup_elem(&tsc_pfx6, &pkey);
        }
        if (pv)
            shared_conn_count_record(&pv->state, now, timeout_ns);
...
static __always_inline void tcp_account_activity_v4(
    struct flow_key *key, __u64 now, __u32 dest_port)
{
#if AUTO_XDP_TCP_ACCOUNT_L3
    struct tcp_src_conn_key_v4 skey;
...
static __always_inline void tcp_account_activity_v6(
    struct flow_key *key, __u64 now, __u32 dest_port)
{
#if AUTO_XDP_TCP_ACCOUNT_L3
    struct tcp_src_conn_key_v6 skey;
...
static __always_inline void tcp_account_close_v4(
    struct flow_key *key, __u64 now, __u32 dest_port)
{
#if AUTO_XDP_TCP_ACCOUNT_L3
    struct tcp_src_conn_key_v4 skey;
...
static __always_inline void tcp_account_close_v6(
    struct flow_key *key, __u64 now, __u32 dest_port)
{
#if AUTO_XDP_TCP_ACCOUNT_L3
    struct tcp_src_conn_key_v6 skey;
...
{
    if (key->family == CT_FAMILY_IPV4)
        tcp_account_established_v4(key, now, dest_port, cfg);
    else
        tcp_account_established_v6(key, now, dest_port, cfg);
    return 0;
}

TCP_ACCOUNTING_ATTR int tcp_src_conn_record_activity(
    struct flow_key *key, __u64 now, __u32 dest_port)
{
    if (key->family == CT_FAMILY_IPV4)
        tcp_account_activity_v4(key, now, dest_port);
    else
        tcp_account_activity_v6(key, now, dest_port);
    return 0;
}

TCP_ACCOUNTING_ATTR int tcp_src_conn_record_close(
    struct flow_key *key, __u64 now, __u32 dest_port)
{
    if (key->family == CT_FAMILY_IPV4)
        tcp_account_close_v4(key, now, dest_port);
    else
        tcp_account_close_v6(key, now, dest_port);
    return 0;
}
#endif
