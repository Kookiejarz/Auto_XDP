...
test_extract_verifier_metrics_sums_subprograms_and_accepts_stack_offset() (
    local output expected
    output=$(
        bash "$REPO_ROOT/tests/bash/extract_verifier_metrics.sh" "subprog" <<'EOF_METRICS'
libbpf: sec '.text': found program 'shared_conn_count_update' at insn offset 0 (0 bytes), code size 95 insns (760 bytes)
libbpf: sec 'xdp': found program 'xdp_port_whitelist' at insn offset 0 (0 bytes), code size 11298 insns (90384 bytes)
verification time 123 usec
stack depth 288+0
processed 456 insns (limit 1000000) max_states_per_insn 2 total_states 3 peak_states 4 mark_read 0
EOF_METRICS
    )
    expected=$'subprog\t11393\t456\t2\t3\t4\t123\t288'
    assert_eq "$output" "$expected"
)
...
test_ensure_curses_installs_opensuse_capability() (
    ...
    assert_contains "$output" "zypper --non-interactive install -y python3-curses"
)
